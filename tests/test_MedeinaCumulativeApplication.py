...
def testHandlingSingleInteractionDataWriting():
    mca = MedeinaCumulativeApplication("dir")
    mca.linkEvidence = {("Spec2", "Spec1"): [1, 3], ("Spec3", "Spec1"): [4]}
    ...
    fileDumpStruct = mca.handleSingleInteractionEvidence(
        evidencingIDs, invertedLinkIndex, "Spec2", "Spec1"
    )
    assert fileDumpStruct == [
        ["Spec2", "Spec1", "Spec2", "Spec1", "", "", "", 2],
        ["Spec2", "Spec1", "Spec2", "Spec1", "", "", "", 1],
    ]
...
def testOutputGraph():
    ...
    unittest.TestCase().assertCountEqual(
        orderTuples(mca.to_graph().edges), orderTuples(expected)
    )
...
def testBuildingTaxaBasedInteractionsLarge():
    ...
    mca = MedeinaCumulativeApplication("dir")
    expected = {
        "acinonyx jubatus": {"puma concolor": [20, 15], "vulpes vulpes": [33]},
        "canis latrans": {"puma concolor": [12]},
        "canis lupus": {"ursus maritimus": [10]},
        "felis catus": {"vulpes vulpes": [25]},
        "helarctos malayanus": {
            "lynx rufus": [7, 21],
            "panthera tigris": [18],
            "puma concolor": [19],
        ...
        "panthera onca": {"panthera leo": [8, 9]},
        "panthera tigris": {
            "panthera leo": [26, 31],
            "puma concolor": [4, 5, 6],
            "ursus americanus": [32],
        },
        "puma concolor": {"ursus arctos": [1]},
        "ursus americanus": {
            "lynx rufus": [39],
            "panthera onca": [40],
        ...
        "ursus arctos": {"panthera tigris": [2, 3]},
        "ursus maritimus": {"panthera tigris": [38], "ursus arctos": [34]},
        "vulpes vulpes": {
            "canis latrans": [29],
            "panthera onca": [28],
            "puma concolor": [27],
        ...
        "vulpes zerda": {"vulpes vulpes": [23, 24]},
    }
...
def testBuildingTaxaBasedInteractionsSmall():
    ...
    mca = MedeinaCumulativeApplication("dir")
    expected = {
        "acinonyx jubatus": {"panthera leo": [4]},
        "panthera tigris": {"acinonyx jubatus": [3], "puma concolor": [2]},
        "puma concolor": {"ursus arctos": [1]},
    }
    assert mca.buildTaxaBasedInteractions(WebObj, "species") == expected
...
def testInvertingNameTranslatorIncludingInvalid():
    ...
    expected = (
        ...
        ["panthera tigris", "ursus arctos", "ursus maritimus", "ursus americanus",],
    )
    assert mca.invertNameTranslation(outputFromAPI) == expected


@patch.object(
    sys.modules["Medeina.MedeinaCumulativeApplication"], "retrieveTaxonomicDataFromAPI"
)
