...
def testTakeIfMatchesGroup():
    data = [{'family':'felidae','X':'Y','species':'Spec1'}, \
    ...
    expected = 1
    assert takeSpeciesMatchingGroupOnly(group,level,data) == ['Spec1', 'Spec2', 'Spec4']
...
def testIndexToTuples():
    ...
    expected = [['Spec1', 'Spec1', ['Spec1', 'Spec11']], ['Spec2', 'Spec2', ['Spec2']], ['Spec3', 'Spec3', []]]
...
def testTranslationDecisionsExpected():
    ...
    expected = ['chrysomus icterocephalus','curaeus forbesi','chrysomus ruficapillus','amblyramphus holosericeus','nesopsar nigerrimus','gnorimopsar chopi','euphagus cyanocephalus', \
                'gymnomystax mexicanus','agelaioides oreopsar','sturnella militaris','agelaius humeralis','dives warszewiczi','dives atroviolaceus','agelaius tricolor','euphagus carolinus']

    assert decideTranslationOnGroupStats([('icteridae', 4), ('turdidae', 1)],listOfClassifications) == expected
...
def testTranslationDecisionsNotThreeQuarters():
    ...
    assert decideTranslationOnGroupStats([('F1', 6),('F2', 6),('F3', 6)],listOfClassifications) == []
...
def testTranslationDecisionsBorder():
    ...
    assert decideTranslationOnGroupStats([('F1', 6),('F2', 2)],listOfClassifications) == []
...
def testTranslationDecisionsMultiCatException():
    ...
    expected = ['chrysomus icterocephalus','curaeus forbesi','chrysomus ruficapillus','turdus albocinctus','amblyramphus holosericeus']

    assert decideTranslationOnGroupStats([('F1', 5),('F2', 1),('F3', 1),('F4', 1),('F5', 1),('F6', 1)],listOfClassifications) == expected
...
def testEnrichmentOfSpecies(test_patch_retr):
    def dynamicTaxaSelection(speciesNames):
        if 'vulpes vulpes' in speciesNames:
            return {'vulpes vulpes':['vulpes vulpes',{'species':'vulpes vulpes','family':'canidae'}],'canis lupus':['canis lupus',{'species':'canis lupus','family':'canidae'}]}
        elif 'panthera tigris' in speciesNames:
            return {'panthera tigris':['panthera tigris',{'species':'panthera tigris','family':'felidae'}],'felis catus':['felis catus',{'species':'felis catus','family':'felidae'}]}
    ...
    expected = [['Dogs', 'Dogs',[{'family': 'canidae', 'species': 'vulpes vulpes'},{'family': 'canidae', 'species': 'canis lupus'}]], \
                ['Cats','Cats',[{'family': 'felidae', 'species': 'panthera tigris'},{'family': 'felidae', 'species': 'felis catus'}]]]
...
def testEnrichmentOfSpeciesBlank(test_patch_retr):
    def dynamicTaxaSelection(speciesNames):
        if 'vulpes vulpes' in speciesNames:
            return {'vulpes vulpes':['vulpes vulpes',{}],'canis lupus':['canis lupus',{}]}
        elif 'panthera tigris' in speciesNames:
            return {'panthera tigris':['panthera tigris',{}],'felis catus':['felis catus',{}]}
