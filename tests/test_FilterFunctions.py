...
def testFilterlinksByDIds():
    ...
    assert filterLinksMetasByDIds(links,[1]) == {1:{'dId':1},2:{'dId':1},4:{'dId':1}}
...
def testFastAccessTaxaDictConversion():
    ...
    expected = {'family':{'Spec1','Spec2'},'genus':{'Spec3','Spec4'},'order':{'Spec5'}}
...
def testFilteringStringNamesByTaxaConstraints():
    ...
    assert filterStringNamesByTaxaConstraints(speciesStringDict,taxaConstraints,taxa) == expected
    speciesStringDict = {
        'Spec1':1,
        'Spec2':2,
        'Spec3':3,
...
def testInferringRemainingLinks():
    ...
    unaccountedFor = [1,4]
    metas = {1:{},2:{},3:{},4:{},5:{},6:{},7:{}}
    assert inferRemainingLinks(newLinks,unaccountedFor,metas) == {1:{},2:{},3:{},4:{},5:{}}
...
def testFilteringDatasetMetaNonStrict():
    ...
    tag = 'interactionType'
    expected = {3:{'interactionType':'predation'},5:{'interactionType':'predation'},2:{},4:{}}
    assert filterDatasetMetaData(datasetMetas,strict,acceptedList,interactionGenerator,tag) == expected
...
def testFilteringDatasetMetaStrict():
    ...
    acceptedList = ['predation']
    tag = 'interactionType'
    expected = {3:{'interactionType':'predation'},5:{'interactionType':'predation'}}
    assert filterDatasetMetaData(datasetMetas,strict,acceptedList,interactionGenerator,tag) == expected
...
def testFilteringLinkMetaNonString():
    ...
    expected = {3:{'interactionType':'predation'},5:{'interactionType':'predation'}}
    assert takeMatchingFromLinkMetas(linkMetas,obs,interactionGenerator,keyName) == (expected,[2,4])
...
def testSearchingForUnaccountedLinksInDatasetMetas():
    ...
    newLinks = {2:{}}
    linkMetas = {1:{'dId':1},2:{'dId':2},3:{'dId':1},4:{'dId':1},5:{'dId':3}}
    acceptedList = ['predation']
    unaccountedFor = [1,3,4,5]
    tag = 'interactionType'
    expected = ({2: {}, 5: {'dId': 3}}, [])
    assert takeMatchingFromDatasetMetas(datasetMetas,newLinks,linkMetas,acceptedList,unaccountedFor,interactionGenerator,tag) == expected