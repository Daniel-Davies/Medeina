...
def testAddingTaxaExceptionsFresh(patch_write,patch_retr):
    ...
    patch_write.assert_called_with('dir', \
...
def testAddingTaxaExceptionsExisting(patch_write,patch_retr):
    ...
    patch_write.assert_called_with('dir', 
                                   'reorderedTaxaInteractions', 
                                   {'panthera tigris': 
                                        {'consumer': 'family', 'resource': 'genus'}, 
                                    'felis catus': 
                                        {'consumer': 'family', 'resource': 'genus'} 
                                    })
...
def testFilteringByDatasetIdsExpected(patch_retr):
    def retrDynamicReturn(a,b):
        if b == REALNAMES:
            ...
            return {1:{'family':'felidae'},2:{'family':'felidae'},3:{'family':'canidea'}}
        elif b == LINKS:
    ...
    assert newWeb.stringNames == {'felis catus': 1,'panthera tigris':2,'vulpes vulpes':3}
    assert newWeb.taxa == {1:{'family':'felidae'},2:{'family':'felidae'},3:{'family':'canidea'}}
...
def testFilteringByObservationTypeAllOnLinks(patch_retr):
    def retrDynamicReturn(a,b):
        if b == REALNAMES:
            ...
            return {1:{'dId':2,'evidencedBy':'observation'},2:{'dId':1,'evidencedBy':'observation'},3:{'dId':1},4:{'dId':2,'evidencedBy':'inferred'}}
        elif b == DATASETS:
            return {1:{},2:{}}
        return {}
    ...
    assert newWeb.linkMetas == {1: {'dId': 2, 'evidencedBy': 'observation'},2: {'dId': 1, 'evidencedBy': 'observation'},3: {'dId': 1}}
...
def testFilteringByObservationTypeMixedDatasetMetasAndLinkMetas(patch_retr):
    def retrDynamicReturn(a,b):
        if b == REALNAMES:
            ...
            return {1:{'dId':2,'evidencedBy':'observation'},2:{'dId':1},3:{'dId':1},4:{'dId':2,'evidencedBy':'inferred'}}
        elif b == DATASETS:
    ...
    assert newWeb.linkMetas == {1: {'dId': 2, 'evidencedBy': 'observation'}, 2: {'dId': 1}, 3: {'dId': 1}} 
    assert newWeb.interactions == {2: {1: [1, 3], 3: [2]},IDTRACKER: 5}
    assert newWeb.stringNames == {'felis catus': 1,'panthera tigris':2,'vulpes vulpes':3}
    assert newWeb.taxa == {1:{'family':'felidae'},2:{'family':'felidae'},3:{'family':'canidea'}}
...
def testFilteringOnTaxa(patch_retr):
    ...
    assert newWeb.datasetMetas == {1: {}, 2: {}}
    assert newWeb.linkMetas == {1: {'dId': 2, 'evidencedBy': 'observation'}, 3: {'dId': 1}}
