...
def testLocationDataStandardising():
    assert standardiseLocationData("Bristol,UK") == {
        "country": "UK",
        "region": "Bristol",
    }
...
def testCorrectTaxaGap(test_patch):
    test_patch.return_value = {"spec1": 1, "spec2": 2, "spec3": 3}

    assert sorted(
        determineTaxonomicGaps(["spec1", "spec2", "spec4", "spec5"], "tmp")
    ) == sorted(["spec4", "spec5"])
...
def testAdditionOfTaxaInformation(test_patch_write, test_patch_retr):
    ...
    writeTaxonomicInformation([["s1", {"family": "new"}]], "dir", {"s1": 4})
    test_patch_write.assert_called_with(
        "dir", "taxonomicIndex", {4: {"family": "new"}, 1: {}, 2: {}, 3: {}}
    )
...
def testSpeciesNameAdditionToFile(test_patch_write, test_patch_retr):
    ...
    addSpeciesToStringNameMapping([["s1", {"family": "new"}]], "dir")
    test_patch_write.assert_called_with(
        "dir", "speciesStringNames", {"existingName": 1, "s1": 2}
    )
...
def testSavingTranslatedSpecies(
    test_add_taxa, test_add_mapping, test_patch_write, test_patch_retr
):
...
@patch("Medeina.interactionParser.addSpeciesToStringNameMapping")
@patch("Medeina.interactionParser.getTaxaAndValidateNewNames")
def testSavingTranslatedSpeciesBlank(
    test_add_taxa, test_add_mapping, test_patch_write, test_patch_retr
):
    parsedSpecString = {"includeInvalid": True, "storageLocation": "dir"}
    species = []
...
def testWritingInteractionsFresh(test_patch_write, test_patch_retr):
    ...
    test_patch_write.assert_has_calls(
        [
            call("dir", "interactionWeb", {IDTRACKER: 3, 5: {6: [1]}, 7: {8: [2]}}),
            call("dir", "links", {1: {"dId": 1}, 2: {"dId": 1}}),
        ]
    )
...
def testDatasetRecordCreationBlank(test_patch_write, test_patch_retr):
    ...
    test_patch_write.assert_called_with(
        "dir", "datasets", {1: {"interactionType": "Meta1", "evidencedBy": "Meta2"}}
    )
    assert newId == 1
...
def testDatasetRecordCreationExisting(test_patch_write, test_patch_retr):
    ...
    test_patch_write.assert_called_with(
        "dir",
        "datasets",
        {1: {}, 2: {"interactionType": "Meta1", "evidencedBy": "Meta2"}},
    )
    assert newId == 2


def testFilteringUnindexableSpecies():
    speciesMappingToId = {"Spec1": 1, "Spec2": 2, "Spec3": 3}
    species = [("Spec1", "Spec2", {}), ("Spec1", "Spec3", {}), ("Spec3", "Spec4", {})]
    parsedSpecString = {"includeInvalid": True, "storageLocation": "dir"}
    speciesWithIds = filterUnindexableSpecies(
        species, speciesMappingToId, parsedSpecString
    )
