...
from .externalAPIs import (
    translateToSpeciesScientificFormatOnly,
    retrieveTaxonomicDataFromAPI,
)
...
def saveNewData(parsedSpecificationString):
    ...
    stringHeadTailData = filterUnindexableSpecies(
        stringHeadTailData, speciesMappingToId, parsedSpecificationString
    )
...
def filterUnindexableSpecies(
    ...
    return list(
        map(
            lambda x: (speciesMappingToId[x[0]], speciesMappingToId[x[1]], x[2]),
            filter(lambda x: verifyValidInteraction(speciesMappingToId, x), species),
        )
    )
...
def createNewDatasetRecord(parsedSpecificationString):
    ...
    existing = retrieveObjFromStore(
        parsedSpecificationString["storageLocation"], DATASETS
    )
    newId = len(existing) + 1
    existing[newId] = datasetMeta
    writeObjToDateStore(
        parsedSpecificationString["storageLocation"], DATASETS, existing
    )
    return newId
...
def addSpeciesToStringNameMapping(validSpecies, directory):
    ...
    for name, taxa in validSpecies:
        stringNames[name] = len(stringNames) + 1
...
def standardiseLocationData(location):
    ...
    for item in indivLocations:
        try:
            agg.append(pycountry.countries.search_fuzzy(x)[0].name)
