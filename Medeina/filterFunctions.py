...
def filterMetasByObs(linkMetas, obs, datasetMetas, strict):
    newLinks = filterLinkMetaData(
        linkMetas, datasetMetas, obs, strict, obsGenerator, "evidencedBy"
    )
    newDataset = filterDatasetMetaData(
        datasetMetas, strict, obs, obsGenerator, "evidencedBy"
    )
...
def filterMetasByCountry(linkMetas, loc, datasetMetas, strict):
    newLinks = filterLinkMetaData(
        linkMetas, datasetMetas, loc, strict, locGenerator, "location"
    )
    newDataset = filterDatasetMetaData(
        datasetMetas, strict, loc, locGenerator, "location"
    )
    return newLinks, newDataset
...
def filterDatasetMetaData(datasetMetas, strict, acceptedList, generator, tag):
    ...
    for key, val in datasetMetas.items():
        ...
        elif any(generator(val, acceptedList)):
...
def takeMatchingFromLinkMetas(linkMetas, obs, generator, keyName):
    ...
    for key, val in linkMetas.items():
        if keyName not in val:
            unaccountedFor.append(key)
        elif any(generator(val, obs)):
            newLinks[key] = val
...
def takeMatchingFromDatasetMetas(
    ...
    for link in unaccountedFor:
        ...
        if tag not in datasetMetas[indivDId]:
            stillUnnacountedFor.append(link)
        elif any(generator(datasetMetas[indivDId], acceptedList)):
