...
class MedeinaCumulativeApplication:
    ...
    def handlePredatorExceptions(self, WebObj, speciesWithTaxa):
        ...
        for species in exceptions:
            ...
            genericSpeciesAtUserTaxa = set(
                list(
                    map(lambda x: speciesWithTaxa[x][consumerBehaviour], genericSpecies)
                )
            )
            preyOfSpeciesAtTaxa = set(
                list(
                    map(
                        lambda x: taxa[x][consumerBehaviour],
                        interactionWeb.get(stringNames[species], {}).keys(),
                    )
                )
            ) - set([""])
            for potentialPrey in genericSpecies:
                if (
                    speciesWithTaxa[potentialPrey][consumerBehaviour]
                    in preyOfSpeciesAtTaxa
                ):
                    self.addLinkEvidenceForPredExceptionInteraction(
                        species,
                        potentialPrey,
                        interactionWeb,
                        speciesWithTaxa[potentialPrey][consumerBehaviour],
        ...
        for exceptedPrey in predatorBucket:
            ...
            for predator in predatorBucket[exceptedPrey]:
                if predator in genericSpecies:
                    for potentialPrey in genericSpecies:
                        if (
                            speciesWithTaxa[potentialPrey][targetTaxa]
                            == targetTaxaValue
                        ):
                            evidencingIDs = WebObj.interactions[
    ...
    def handleNonExceptionSpecies(self, WebObj, speciesWithTaxa, taxaLevel):
        interactionsAtUserDefinedLevel = self.buildTaxaBasedInteractions(
            WebObj, taxaLevel
        )
        genericSpecies = self.nonExceptedSpecies(WebObj, speciesWithTaxa)
        genericSpeciesAtUserTaxa = set(
            list(map(lambda x: speciesWithTaxa[x][taxaLevel], genericSpecies))
        )
        genericInteractions = []
        ...
        for predator in interactions:
            for prey in interactions[predator]:
                ...
                if not (
                    ...
                    or len(stringResource.get(prey, "")) == 0
                ):
                    taxaBasedInteractions[stringResource[predator]][
                        stringResource[prey]
                    ].extend(sourceList)
        deleteList = []
    ...
    def to_list_original(self):
        ...
        return list(
            map(
                lambda x: (
                    (x[0], x[1]),
                    (self.scientificToUser[x[0]], self.scientificToUser[x[1]]),
                ),
                baseList,
            )
        )
    ...
    def to_matrix_original(self):
        ...
        nodeOrder = list(
            map(lambda x: self.scientificToUser[x], self.interactionsToNodes())
        )
        return nx.to_numpy_matrix(G, nodelist=nodeOrder), nodeOrder
    ...
    def audit(self, filepath=None):
        ...
        for predator, prey in self.linkEvidence.keys():
            ...
            fileDumpStruct.extend(
                self.handleSingleInteractionEvidence(
                    evidencingIDs, invertedLinkIndex, predator, prey
                )
            )
    ...
    def handleSingleInteractionEvidence(
        self, evidencingIDs, invertedLinkIndex, predator, prey
    ):
        fileDumpStruct = []
        for idx in evidencingIDs:
            ...
            interactionType = (
                linkMetaData["interactionType"]
                if "interactionType" in linkMetaData
                else datasetMetaData.get("interactionType", "")
            )
            evidencedBy = (
                linkMetaData["evidencedBy"]
                if "evidencedBy" in linkMetaData
                else datasetMetaData.get("evidencedBy", "")
            )
            location = (
                linkMetaData["location"]
                if "location" in linkMetaData
                else datasetMetaData.get("location", "")
            )
            ...
            for prey in self.interactionWeb[predator]:
                ...
                for l in linkIds:
                    if l in linksOfInterest:
                        linkIndex[l] = (
                            self.stringNames[predator],
                            self.stringNames[prey],
                        )
    ...
    def summary(self):
        ...
        print(
            "Captured "
            + str(self.lenExcepted)
            + " interactions from taxonomic exceptions"
        )
        print(
            "Involvement of "
            + str(len(set(itertools.chain(*self.linkEvidence.keys()))))
            + " user provided species captured"
        )
        if self.speciesLen == 0:
            print(
                "Percentage of user provided species involved in this application: 0%"
            )
            return
        print(
